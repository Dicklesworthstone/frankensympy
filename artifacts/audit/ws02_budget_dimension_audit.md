...
**Closure finding (RESOLVED 2026-09-17, PlumSnow):** evidence-class and non-evidence-outcome *names* match 7/7 and 8/8, respectively. The one property gap — `registries/evidence_classes.toml:58` requires `UserAsserted.can_discharge_exact_equality = "inside_declared_context_only"` while `fsym-outcome` returned `No` — is repaired in source: `Discharge::InsideDeclaredContextOnly` added with exact registry spelling in `as_str`/`parse`, `UserAsserted` maps to it, regression assertions pin the roundtrip, the distinction from unconditional `Yes`, and the continued rejection of `UserAsserted` by `MathOutcome::established`. Gates observed: `cargo test -p fsym-outcome` 15/15, `cargo test -p fsym-outcome -p fsym-budget` 35/35, workspace check and clippy `-D warnings` clean, `cargo fmt --check` clean. All RCH invocations selected workers but fell back locally (`RCH-E415`, workspace-inherited `serde` lacks enclosing workspace); recorded as local-fallback evidence, not remote execution. Per the bead's independent-gate rule, closure remains reserved for a reviewer other than the repair author.
