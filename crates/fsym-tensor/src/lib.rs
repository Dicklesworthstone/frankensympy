...
mod tests {
    ...
    fn test_metric_index_raising_lowering_rejects_rank_variance_and_dimension() {
        ...
        .unwrap();
        assert_eq!(
            eta.lower_vector(&rank2),
            Err(TensorError::RankMismatch(1, 2))
        );
        assert_eq!(
            eta.raise_covector(&rank2),
            Err(TensorError::RankMismatch(1, 2))
        );
        ...
        let covec = TensorExpr::with_components(
            "w",
            4,
            vec![TensorIndex::lower("mu")],
            vec![Expr::from_i64(1); 4],
        )
        .unwrap();
        assert!(matches!(
            eta.lower_vector(&covec),
            Err(TensorError::IndexMismatch(_))
        ));
        let vec = TensorExpr::with_components(
            "v",
            4,
            vec![TensorIndex::upper("mu")],
            vec![Expr::from_i64(1); 4],
        ...
        let three_covec = TensorExpr::with_components(
            "u",
            3,
            vec![TensorIndex::lower("i")],
            vec![Expr::from_i64(1), Expr::from_i64(2), Expr::from_i64(3)],
        )
        .unwrap();
        assert_eq!(
