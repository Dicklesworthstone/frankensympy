...
use fsym_proof_kernel::{Claim, claim_verification_units, verify_derivation_independent};
use serde::{Deserialize, Deserializer, Serialize, Serializer};
use std::collections::HashMap;
use thiserror::Error;

const MAX_CANDIDATE_NAMESPACE_ENTRIES: usize = 4_096;
const MAX_VERIFIED_NAMESPACE_ENTRIES: usize = 4_096;
...
pub enum NamespaceError {
    ...
    #[error("Candidate claim exceeds trusted verification bounds: {0}")]
    InvalidCandidate(String),
    #[error("Candidate namespace exceeds its entry limit of {limit}")]
    CandidateLimitExceeded { limit: usize },
    #[error("Verified namespace exceeds its entry limit of {limit}")]
    VerifiedLimitExceeded { limit: usize },
}
...
impl<'de> Deserialize<'de> for CandidateNamespace {
        ...
        D: Deserializer<'de>,
    {
        ...
        if wire.candidates.len() > MAX_CANDIDATE_NAMESPACE_ENTRIES {
            return Err(serde::de::Error::custom(format!(
                "candidate namespace exceeds its entry limit of {MAX_CANDIDATE_NAMESPACE_ENTRIES}"
            )));
        }
        let mut candidates = HashMap::with_capacity(wire.candidates.len());
        for claim in wire.candidates {
            claim_verification_units(&claim).map_err(|error| {
                serde::de::Error::custom(format!("candidate claim rejected: {error}"))
            })?;
...
impl CandidateNamespace {
    ...
    pub fn register_candidate(&mut self, claim: Claim) -> Result<[u8; 32], NamespaceError> {
        claim_verification_units(&claim)
            .map_err(|error| NamespaceError::InvalidCandidate(error.to_string()))?;
        let digest = claim.digest();
        if !self.candidates.contains_key(&digest)
            && self.candidates.len() >= MAX_CANDIDATE_NAMESPACE_ENTRIES
        {
            return Err(NamespaceError::CandidateLimitExceeded {
                limit: MAX_CANDIDATE_NAMESPACE_ENTRIES,
            });
        }
...
impl VerifiedNamespace {
    ...
    pub fn promote_candidate(
        ...
        if !self.verified.contains_key(candidate_digest)
            && self.verified.len() >= MAX_VERIFIED_NAMESPACE_ENTRIES
        {
            return Err(NamespaceError::VerifiedLimitExceeded {
                limit: MAX_VERIFIED_NAMESPACE_ENTRIES,
            });
        }
    ...
    pub fn insert_verified(
        ...
        let verified = independently_verify_envelope(envelope, context)?;
        // Digest only after bounded structural preflight and independent replay. Hashing an
        // untrusted recursive claim before that gate would itself be an unbounded trust boundary.
        let claim_digest = verified.envelope.claim.digest();
        if !self.verified.contains_key(&claim_digest)
            && self.verified.len() >= MAX_VERIFIED_NAMESPACE_ENTRIES
        {
            return Err(NamespaceError::VerifiedLimitExceeded {
                limit: MAX_VERIFIED_NAMESPACE_ENTRIES,
...
mod tests {
    ...
    fn promotion_requires_matching_receipt_and_integrity() {
        ...
        let digest = candidates.register_candidate(claim.clone()).unwrap();
    ...
    fn promotion_with_mismatched_receipt_is_rejected() {
        ...
        let digest_a = candidates.register_candidate(claim_a).unwrap();
    ...
    fn candidate_namespace_has_deterministic_json_roundtrip() {
        ...
        first
            .register_candidate(Claim::equality(Expr::symbol("z"), Expr::symbol("z")))
            .unwrap();
        first
            .register_candidate(Claim::equality(Expr::symbol("a"), Expr::symbol("a")))
            .unwrap();
    ...
    #[test]
    fn candidate_namespace_refuses_unbounded_growth() {
        let mut candidates = CandidateNamespace::new();
        for index in 0..MAX_CANDIDATE_NAMESPACE_ENTRIES {
            let symbol = Expr::symbol(format!("x{index}"));
            candidates
                .register_candidate(Claim::equality(symbol.clone(), symbol))
                .unwrap();
        }

        let overflow = Expr::symbol("overflow");
        assert_eq!(
            candidates.register_candidate(Claim::equality(overflow.clone(), overflow)),
            Err(NamespaceError::CandidateLimitExceeded {
                limit: MAX_CANDIDATE_NAMESPACE_ENTRIES,
            })
        );
    }
